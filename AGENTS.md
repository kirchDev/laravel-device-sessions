...
- The order in `DeviceSessionsServiceProvider::MIGRATIONS` is **load-bearing**, not cosmetic. The remember-token table carries a foreign key to `user_devices`, and each entry is published one second after the previous one so the consumer's migrator sorts them that way. Publishing both under the same timestamp puts the migrator back on alphabetical order, where `create_user_device_remember_tokens_table` sorts _before_ `create_user_devices_table` and the foreign key breaks. Give every new migration its place in that list.
- The **source** filenames in `database/migrations/` keep their `0001_01_01_*` prefixes purely as an internal sort key for `migrate --path` in the suite. Consumers never see those names, so the prefix no longer decides where the tables land in a host app's timeline.
