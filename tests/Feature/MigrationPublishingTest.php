...
it('hands the consumer a second migration when a source migration is renamed', function () {
    ...
    $renamed = makeTemporaryDirectory('00001_create_device_sessions_user_devices_table.php');
    $target = array_values(PackageMigrations::publishMap($renamed, 1_600_000_000))[0];

    touch($target);

    expect(glob($database.'/migrations/*_user_devices_table.php'))->toHaveCount(2);
