...
function packageTables(): array
{
    ...
    $tables = config('device-sessions.table_names');

    // 'users' is the host application's table, not one the package migrations create.
    return [$tables['devices'], $tables['remember_tokens']];
}
...
{
    return ServiceProvider::pathsToPublish(DeviceSessionsServiceProvider::class, 'device-sessions-migrations');
}
...
function publishAgainst(Application $app, string ...$alreadyPublished): string
{
    ...
    (new DeviceSessionsServiceProvider($app))->register()->boot();
...
it('names every source migration with a timestamp prefix the publish strips', function () {
    ...
    // the package path, where an unprefixed name would run before create_user_devices_table.
...
it('hands the consumer a second migration when a source migration is renamed', function () {
    ...
    $stale = '2020_01_01_000000_create_device_sessions_user_devices_table.php';
    $database = publishAgainst($this->app, $stale);

    $devices = publishedBySource()['0001_01_01_000001_create_user_devices_table.php'];

    expect(basename($devices))->not->toBe($stale);

    touch($devices);

    expect(glob($database.'/migrations/*_create_*user_devices_table.php'))->toHaveCount(2);

    removeTemporaryDirectory($database);
});

it('reuses an already published migration instead of stamping a second copy', function () {
    $existing = '2020_01_01_000000_create_user_devices_table.php';
    $database = publishAgainst($this->app, $existing);

    expect(publishedBySource()['0001_01_01_000001_create_user_devices_table.php'])
...
it('does no per-migration work outside the console', function () {
    ...
    $spy = function () {
        return new class($this->app) extends DeviceSessionsServiceProvider
        {
